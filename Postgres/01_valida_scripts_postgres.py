...
class validaScripts:
    ...
    def validate_script_name(self, script):
        pattern = r'^(\d+)-([RDS]+)-([A-Z_]+)-([A-Z_]+)-(INSERT|UPDATE|DELETE|SELECT|ANONIMO)-([a-zA-Z0-9_]+)\.sql$'
        ...
        if not match:
            ...
            if len(parts) != 6 or not parts[-1].endswith('.sql'):
                ...
                if not re.match(r'^[A-Z_]+$', parts[2]):
    ...
    def validate_specific_schema_in_sql(self, statement, schema):
        ...
        table_name = table_name.strip('`"')  # Limpia el nombre de la tabla
    ...
    def menu(self):
        self.allowed_host = os.environ.get("allowed_host_postgres").split(", ")
        self.allowed_instance = os.environ.get("allowed_instance_postgres").split(", ")
        self.allowed_schema = os.environ.get("allowed_schema_postgres").split(", ")
